#' @title PackageGroup9
#'
#' @description THis package is a test package
#'
#' @param symbol
#'
#' @return NULL
...
confint.myLm=function(x,alpha=0.05,approach="asymp"){
  ...
  if(approach=="asymp"){
    ...
    ci.betas[1,]=quantile(betaMatrix[,1],probs=c(1-quant,quant),na.rm = TRUE)
    ci.betas[2,]=quantile(betaMatrix[,2],probs=c(1-quant,quant),na.rm=TRUE)
